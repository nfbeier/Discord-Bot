...
class users(commands.Cog):
    ...
    async def volume(self, ctx,volume: float):
        oldVolume = await self.pull_value(ctx.author.guild,ctx.author,'volume')
        await self.set_value(ctx.author.guild,ctx.author,'volume',volume)
        await ctx.send('Volume set from %0.3f to %0.3f for %s'%(oldVolume,volume,ctx.author.name))

    @commands.command()
    async def solo_play(self,ctx,):
        val = await self.pull_value(ctx.author.guild,ctx.author,'solo_play')
        await self.set_value(ctx.author.guild,ctx.author,'solo_play',val==0)
    ...
    async def pull_user(self,guild,member):
        testRow = await self.bot.db.fetchrow("SELECT * FROM %s WHERE guild_id = %d"%(table_name,guild.id))
        return testRow
